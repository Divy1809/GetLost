...
router.post('/', (req, res) => {
  ...
  const checkQuery = `
    ...
    WHERE from_user_id = ? AND to_user_id = ? AND post_id = ?
  `;

  db.query(checkQuery, [fromUserId, toUserId, postId], (err, existing) => {
    if (err) {
    ...
    db.query(insertQuery, [fromUserId, toUserId, postId, message], (err, result) => {
      if (err) {
...
router.delete('/:requestId', (req, res) => {
  ...
  const query = `DELETE FROM connection_requests WHERE id = ?`;
...
router.get('/connections/:userId', (req, res) => {
  ...
  const query = `
    SELECT 
      ...
      cr.created_at,
      CASE 
        WHEN cr.from_user_id = ? THEN u2.name
        ELSE u1.name
      END as connection_name,
      CASE 
        WHEN cr.from_user_id = ? THEN u2.email
        ELSE u1.email
